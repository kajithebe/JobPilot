export default function ExecutiveTemplate({ content, themeConfig, sectionOrder }) {
  ...
  return (
    ...
    >
      ...
      <div className="px-10 py-10 text-white" style={{ backgroundColor: theme.primaryColor }}>
        <h1 className="text-4xl font-bold tracking-tight mb-1">
          {content?.personalInfo?.fullName || 'Your Name'}
        </h1>
        <div className="w-16 h-0.5 my-3" style={{ backgroundColor: 'rgba(255,255,255,0.4)' }} />
        <div className="flex flex-wrap gap-4 text-[10px] text-white/70">
        ...
        {content?.personalInfo?.summary && (
          <p className="text-[10px] text-white/60 mt-4 leading-relaxed max-w-2xl">
            {content.personalInfo.summary}
          </p>
        )}
      </div>
              ...
              {content.experience.map((item, i) => (
                <div key={i}>
                  <div className="flex justify-between items-start">
                    ...
                    <p className="text-[9px] text-gray-400 whitespace-nowrap ml-4 mt-0.5 font-medium">
                      {item.startDate}
              ...
              {content.education.map((item, i) => (
                <div key={i} className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-600 mt-0.5">{item.institution}</p>
                    {item.gpa && <p className="text-[9px] text-gray-400 mt-0.5">GPA: {item.gpa}</p>}
                  </div>
                  <p className="text-[9px] text-gray-400 whitespace-nowrap ml-4 mt-0.5">
                    {item.startDate}
                    {item.endDate ? ` – ${item.endDate}` : ''}
                  </p>
                </div>
              ...
              {content.certifications.map((item, i) => (
                <div key={i} className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-600">{item.issuer}</p>
                  </div>
                  <p className="text-[9px] text-gray-400 whitespace-nowrap ml-4">{item.date}</p>
                </div>
              ))}
            </div>
          </div>
